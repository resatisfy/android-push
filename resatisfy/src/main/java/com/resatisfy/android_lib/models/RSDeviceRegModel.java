package com.resatisfy.android_lib.models;

public class RSDeviceRegModel {
    private String status;
    public String getStatus() { return this.status; }
    public void setStatus(String msg) { this.status = status; }

    private String msg;
    public String getMsg() { return this.msg; }
    public void setMsg(String msg) { this.msg = msg; }

    private String channelId;
    public String getChannelId() { return this.channelId; }
    public void setChannelId(String channelId) { this.channelId = channelId; }

    private String appKey;
    public String getAppKey() { return this.appKey; }
    public void setAppKey(String appKey) { this.appKey = appKey; }

    private String appSecret;
    public String getAppSecret() { return this.appSecret; }
    public void setAppSecret(String appSecret) { this.appSecret = appSecret; }

    private String deviceToken;
    public String getDeviceToken() { return this.deviceToken; }
    public void setDeviceToken(String deviceToken) { this.deviceToken = deviceToken; }

    private String deviceType;
    public String getDeviceType() { return this.deviceType; }
    public void setDeviceType(String deviceType) { this.deviceType = deviceType; }

    private String iosAndroidId;
    public String getIosAndroidId() { return this.iosAndroidId; }
    public void setIosAndroidId(String iosAndroidId) { this.iosAndroidId = iosAndroidId; }

    private String fcmSenderId;
    public String getFcmSenderId() { return this.fcmSenderId; }
    public void setFcmSenderId(String fcmSenderId) { this.fcmSenderId = fcmSenderId; }


}
